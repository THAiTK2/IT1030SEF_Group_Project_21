...
function showScreen(screenId) {
  // Safety check: only allow known screens
  var isValidScreen = coreScreens.includes(screenId);
  if (!isValidScreen) {
    return;
  }

  // Show only the selected screen
  for (var i = 0; i < refs.screens.length; i += 1) {
    var oneScreen = refs.screens[i];
    var oneScreenId = oneScreen.dataset.screen;

    // Keep quick-actions overlay logic unchanged
    if (oneScreenId === "quick-actions-overlay") {
      continue;
    }

    if (oneScreenId === screenId) {
      oneScreen.classList.add("screen-current");
    } else {
      oneScreen.classList.remove("screen-current");
    }
  }
  ...
  // Bottom navigation is visible only in main app tabs
  var mainTabs = ["home", "workouts", "metrics", "profile"];
  var shouldShowBottomNav = mainTabs.includes(screenId);
  ...
  // Close quick actions when leaving home
  if (appState.isQuickActionsOpen && screenId !== "home") {
    closeQuickActions();
  }

  // Close workout video when leaving workouts
  if (screenId !== "workouts" && appState.isWorkoutVideoOpen) {
    closeWorkoutVideo();
  }

  // Render metrics chart only when entering metrics screen
  if (screenId === "metrics") {
    setTimeout(function () {
